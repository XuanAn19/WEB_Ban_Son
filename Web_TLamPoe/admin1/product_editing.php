        ...
        <div id="content-box">
            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($kn, "UPDATE `product` SET `ten` = '" . $_POST['name'] . "',`quantity` = " . str_replace('.', '', $_POST['quantity']) . ",`images` =  '" .$image. "', `price` = " . str_replace('.', '', $_POST['price']) . ", `content` = '" . $_POST['content'] . "', `last-updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($kn, "INSERT INTO `product` (`id`, `ten`,`quantity`, `images`, `price`, `content`, `created-time`, `last-updated`) VALUES (NULL, '" . $_POST['name'] . "','" . str_replace('.', '', $_POST['quantity']) . "','" .$image. "', '" . str_replace('.', '', $_POST['price']) . "', '" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
                        ...
                        if (!$result) { //Nếu có lỗi xảy ra
                            ...
                            if (!empty($galleryImages)) {
                                $product_id = ($_GET['action'] == 'edit' && !empty($_GET['id'])) ? $_GET['id'] : $kn->insert_id;
                                $insertValues = "";
                                ...
                                $result = mysqli_query($kn, "INSERT INTO `image_library`(`id`, `product_id`, `path`, `created-time`, `last-updated`) VALUES " . $insertValues . ";");
                            }
                        }
                    }
                } else {
                ...
                if (!empty($_GET['id'])) {
                    ...
                    $gallery = mysqli_query($kn, "SELECT * FROM `image_library` WHERE `product_id` = " . $_GET['id']);
                ...
                <form id="product-form" method="POST" action="<?= (!empty($product) && !isset($_GET['task'])) ? "?action=edit&id=" . $_GET['id'] : "?action=add" ?>"  enctype="multipart/form-data">
                    ...
                    <div class="wrap-field">
                        ...
                        <input type="text" name="name" value="<?= (!empty($product) ? $product['ten'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
					<div class="wrap-field">
