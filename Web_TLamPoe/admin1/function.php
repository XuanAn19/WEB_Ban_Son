...
function checkPrivilege($uri = false) {
    $uri = $uri != false ? $uri : $_SERVER['REQUEST_URI'];//nếu $uri khác false thì giữ nguyên giá trị là $uri, 
															//còn mà bằng false thì là dùng $_SERVER['REQUEST_URI'] 
    if(empty($_SESSION['username']['privileges'])){
        return false;
    }
    $privileges = $_SESSION['username']['privileges'];
    $privileges = implode("|", $privileges);
    preg_match('/Tin\.php$|' . $privileges . '/', $uri, $matches);
	//vd account_listing\.php$ | Tin\.php$ :
	//Sử dụng hàm preg_match() để tìm kiếm liệu đường dẫn $uri có phù hợp với biểu thức chính quy '/Tin.php$|' . $privileges . '/' hay không. Biểu thức chính quy này cho phép kiểm tra xem đường dẫn có kết thúc bằng "Tin.php" hoặc có chứa các giá trị trong chuỗi $privileges hay không.
	//Nếu hàm preg_match() tìm thấy phù hợp, nghĩa là người dùng có quyền truy cập vào trang web hiện tại, hàm sẽ trả về giá trị true. Ngược lại, nếu không tìm thấy phù hợp, hàm sẽ trả về giá trị false.
...
function processUploadFile($file, $uploadPath) {
    ...
    if ($file != false) {
        ...
        if (move_uploaded_file($file["tmp_name"], $uploadPath . '/' . $file["name"])) {//move_uploaded_file để di chuyển tệp tin vào thư mục chỉ định
            return array(
                'error' => false,
...
function validateUploadFile($file, $uploadPath) {
    ...
    $fileType = strtolower(substr($file['name'], strrpos($file['name'], ".") + 1));//đưa về chữ thường
...
define ('BASE_URL','http://localhost/Web_TLam/');

	function redirec_to($page="logindnweb.php"){
	$url=BASE_URL.$page;
	header("location:$url");
	}
	function is_login(){
